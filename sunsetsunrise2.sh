...
sunrise=$(curl -sf "https://api.sunrise-sunset.org/json?lat=46.745995&lng=7.122781&date=today" 2>/dev/null| jq -r '.results.sunrise')
...
sunset=$(curl -sf "https://api.sunrise-sunset.org/json?lat=46.745995&lng=7.122781&date=today" 2>/dev/null| jq -r '.results.sunset')
if [ "$sunset" != "null" ]; then
